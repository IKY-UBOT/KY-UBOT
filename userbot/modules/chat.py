...
async def log(log_text):
    ...
    if BOTLOG:
        if log_text.reply_to_msg_id:
            ...
            await log_text.edit("`Apa Yang Musti Gw Log?`")
            return
...
@dior_cmd(pattern="kickme$")
async def kickme(leave):
    """Basically it's .kickme command"""
    await leave.edit(
        f"**{ALIVE_NAME} Keluar Dari Grup, Dahh anak anak kontol!!**"
    )
    await leave.client.kick_participant(leave.chat_id, "me")


@dior_cmd(pattern="unmutechat$")
async def unmute_chat(unm_e):
    ...
    await unm_e.edit("```Iya udah iya gw bukain, Obrolan Tidak Lagi Dibisukan```")
    await sleep(2)
    await unm_e.delete()


@dior_cmd(pattern="mutechat$")
async def mute_chat(mute_e):
    ...
    if BOTLOG:
        await mute_e.client.send_message(
            BOTLOG_CHATID, str(mute_e.chat_id) + " Gw mute Lo bedebah."
        )
...
@dior_cmd(pattern="regexninja (on|off)$")
...
async def get_chatinfo(event):
    ...
    try:
        ...
        try:
            ...
            await event.edit(
                "`Ini Adalah Group/Channel Privasi Atau Mungkin Anda Telah Terbanned Dari Sana`"
            )
            return None
